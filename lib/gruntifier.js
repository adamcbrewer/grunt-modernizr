...
var Gruntifier = function (grunt, target, done) {
	...
	// run the specified target if provided
	if (target) config = config[target];
	...
	_Gruntifier.prototype = {
		init : function () {
			...
			// for chche busting locally downloaded files
			this.bust = config.bust || false;
		...
		crawlFilesForTests : function (tests) {
			...
			if (config.files.dev !== "remote") {
				if (!fs.existsSync(config.files.dev)) {
					grunt.fail.warn([
						"Can't find your Modernizr development build at " + config.files.dev,
					...
					config.files.src.push("!" + config.files.dev);
				}
			}

			// Also exclude generated file
			config.files.src.push("!" + config.files.dest);
		...
		xhr : function (pathname) {
			...
			if (pathname.length) {
				for (i = 0, j = pathname.length; i < j; i++) {
					...
					if (!this.bust && fs.existsSync(cachePath)) {
		...
		finalize : function (data, tests, customTests) {
			...
			// Write!
			grunt.file.write(config.files.dest, build);

			// All set.
			if (!_quiet) {
				grunt.log.writeln();
				grunt.log.ok("Wrote file to " + config.files.dest);
			}
